...
func applyReconcilation(
    ...
    case let (.root(changes), .node(_, _, oldChildTree)):
        ...
        if let component = renderTree.renderable as? BaseComponent {
            shouldUpdate = component.shouldUpdate(component)
        }
        if shouldUpdate {
            ...
            if case let .node(renderable, view, childTree) = newRenderTree! {
                ...
                for (index, change) in changes.enumerated() {
                    ...
                    if case .update = change {
                        ...
                        var shouldUpdate = true
                        if let component = childTree[index].renderable as? BaseComponent {
                            shouldUpdate = component.shouldUpdate(component)
                        }
                        if shouldUpdate{
